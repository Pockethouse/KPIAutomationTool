...
public class Format
{
    ...
    public void AddResultsToSheet<T1, T2>(IEnumerable<T1> results1, IEnumerable<T2> results2, string sheetName)
    {
        using (var package = new ExcelPackage(new FileInfo(_filePath)))
        {
            ...
            // Add header row for results1
            for (int i = 0; i < properties1.Length; i++)
            {
                worksheet.Cells[1, i + 1].Value = properties1[i].Name;
            }

            // Add header row for results2, starting after the last column of results1
            for (int i = 0; i < properties2.Length; i++)
            {
                worksheet.Cells[1, properties1.Length + i + 1].Value = properties2[i].Name;
            }

            int row = 2;
            int colOffset = properties1.Length + 1;
            ...
            row = 2; // Reset row counter for results2
            ...
            foreach (var result in results2)
            {
                ...
                {
                    worksheet.Cells[row, colOffset + col].Value = properties2[col].GetValue(result);
                }
